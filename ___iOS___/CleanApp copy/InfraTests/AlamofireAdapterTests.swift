    ...
    func post(to url: URL, with data: Data?, completion: @escaping (Result<Data?, HttpError>) -> Void ) {
        session.request(url, method: .post, parameters: data?.toJson(), encoding: JSONEncoding.default).responseData {
            ...
            case .success(let data):
                ...
                case 401:
                    completion(.failure(.unathorized))
                case 403:
...
class AlamofireAdapterTests: XCTestCase {
    ...
    func test_post_should_complete_with_error_when_requested_complete_with_error() {
    ...
    func test_post_should_complete_with_data_when_requested_complete_with_200() {
        expectResult( .success(makeValidData()), when: (data: makeValidData(), response: makeHttpResponse(), error: nil) )
    }
    
    func test_post_should_complete_with_data_when_requested_complete_with_204() {
    ...
    func test_post_should_complete_with_error_when_requested_complete_with_non_200() {
        ...
        expectResult( .failure(.unathorized), when: (data: makeValidData(), response: makeHttpResponse(statusCode: 401), error: nil) )
...
extension AlamofireAdapterTests {
    ...
    func expectResult(_ expectedResult: Result<Data?, HttpError>, when stub: (data: Data?, response: HTTPURLResponse?, error:Error?), file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.post(to: makeUrl(), with: makeValidData()) { receivedResult in
            ...
            default: XCTFail("Expected error got \(receivedResult) instead", file: file, line: line)
            }
            exp.fulfill()
        }
        wait(for: [exp], timeout: 1)
